pipeline {
  ...
  environment {
    GIT_REPO_URL = 'git@github.com:zchelalo/neuraclinic-location.git'
  ...
  stages {
    ...
    stage('Go pipeline') {
      ...
      stages {
        ...
        stage('Build') {
          steps {
            sh 'mkdir -p dist && go build -buildvcs=false -trimpath -o dist/neuraclinic-location ./cmd'
            sh 'go build -buildvcs=false -trimpath -o dist/location-import ./cmd/location-import'
          }
          post {
            always {
    ...
    stage('Docker Build') {
      steps {
        sh 'docker build -f .docker/Dockerfile -t neuraclinic-location:${BUILD_NUMBER} .'
      }
    }
        ...
        withSonarQubeEnv(credentialsId: 'sonar-token', installationName: 'SonarQube') {
          sh '''
            ${SONAR_SCANNER_HOME}/bin/sonar-scanner \
              -Dsonar.projectKey=neuraclinic-location \
              -Dsonar.sources=. \
              -Dsonar.exclusions=gen/**,dist/**,certs/**,data/** \
