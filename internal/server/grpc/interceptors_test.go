...
func TestUnaryInterceptorStoresNormalizedLanguage(t *testing.T) {
	...
	interceptor := UnaryInterceptor(zap.NewNop(), "location")
	ctx := metadata.NewIncomingContext(context.Background(), metadata.Pairs("accept-language", "es-MX,en;q=0.8"))

	_, err := interceptor(ctx, nil, &grpc.UnaryServerInfo{FullMethod: "/location.v1.LocationService/ListCountries"}, func(ctx context.Context, _ any) (any, error) {
