...
func (uc *UseCase) Execute(ctx context.Context, filter domain.SettlementFilter) ([]domain.Settlement, error) {
	...
	filter.LocalityCode = appshared.NormalizeCode(filter.LocalityCode)
	...
	return uc.repo.ListSettlements(ctx, filter)
}
